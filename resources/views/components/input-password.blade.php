...
<div class="relative">
<input {{$attributes}} :type="!show ? 'password' : 'text'" name="{{$name}}" id="{{$name}}" 
value="{{ 
$value }}"
       class="shadow-sm border-gray-300 rounded-md  bg-gray-50
       appearance-none border  w-full py-2 px-3  leading-tight
        focus:outline-none focus:shadow-outline "
       placeholder="{{$placeholder ?? $label}}..." />
       <button type="button" id="show-{{$name}}" @click="show=!show" class=" absolute right-3 top-1 ">

        <svg xmlns="http://www.w3.org/2000/svg" fill="none"
         x-show="show" viewBox="0 0 
      ...
      <svg xmlns="http://www.w3.org/2000/svg" x-show='!show' x-cloak
       fill="none"
